...
from info import ADMINS, AUTH_CHANNEL, AUTH_USERS, AUTH_GROUPS, P_TTI_SHOW_OFF, IMDB, \
    SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE, BUTTON, CUSTOM_FILE_CAPTION, DOWNLOAD_TEXT_URL, DOWNLOAD_TEXT_NAME
...
async def next_page(bot, query):
    ...
    if ENABLE_SHORTLINK == True:
        if settings['button']:
            btn = [
                [
                    InlineKeyboardButton(
                        text=f"💝 [{get_size(file.file_size)}] 💖 {file.file_name}", url=await get_shortlink(query.message.chat.id, f"https://telegram.dog/{temp.U_NAME}?start=aNsH_{file.file_id}")
                    ),
                ]
                for file in files
            ]
            ...
            btn.insert(0, 
                [
                    InlineKeyboardButton("✅ How To Download ✅", url="https://t.me/+SAO4zXgp9YdiZDg9")
                ])
    else:
        if settings['button']:
            btn = [
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                await client.send_message(chat_id=query.from_user.id, text="<b>For daily Shopping Deals, Loots and Offers on Amazon, Flipkart and more</b>\n\n<b>Join 👉 @dealsandgadgets </b>")
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("Pahle Channel ko join krke aao fir refresh.", show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
    ...
    if ENABLE_SHORTLINK == True:
        if settings["button"]:
            btn = [
                [
                    InlineKeyboardButton(
                        text=f"💝 [{get_size(file.file_size)}] 💖 {file.file_name}", url=await get_shortlink(message.chat.id, f"https://telegram.dog/{temp.U_NAME}?start=aNsH_{file.file_id}")
                    ),
                ]
                for file in files
            ]
